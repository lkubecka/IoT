...
int bmp180_read_int16(uint8_t reg)
{
    ...
    if (rc != 0) {
        esp_log_write(ESP_LOG_ERROR, TAG, "Read [%02x] failed rc=%d", reg, rc);
    }
    return data;
}
...
uint8_t bmp180_write(uint8_t reg, uint8_t data)
{
    ...
    if (ret != 0) {
        esp_log_write(ESP_LOG_ERROR, TAG, "Write [%02x]=%02x failed", reg, data);
    }
    return ret;
}
...
esp_err_t bmp180_init(int pin_sda, int pin_scl)
{
    if (portTICK_RATE_MS > 1) {
         ...
         */
        esp_log_write(ESP_LOG_ERROR, TAG, "FreeRTOS tick rate (%d) too slow", portTICK_RATE_MS);
    ...
    if (twi_writeTo(BMP180_ADDRESS, &reg, 1, true) == 0) {
        esp_log_write(ESP_LOG_DEBUG, TAG, "Sensor found at 0x%02x", BMP180_ADDRESS);
        ...
        esp_log_write(ESP_LOG_ERROR, TAG, "Sensor not found at 0x%02x", BMP180_ADDRESS);
        return ESP_ERR_BMP180_NOT_DETECTED;
    }
}
