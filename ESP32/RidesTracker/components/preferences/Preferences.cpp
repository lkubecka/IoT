...
bool Preferences::begin(const char * name, bool readOnly){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_open failed: %s", nvs_error(err));
        return false;
    }
    _started = true;
    return true;
...
bool Preferences::clear(){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_erase_all fail: %s", nvs_error(err));
        return false;
    }
    return true;
}
...
bool Preferences::remove(const char * key){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_erase_key fail: %s %s", key, nvs_error(err));
        return false;
    }
    return true;
}
...
size_t Preferences::putChar(const char* key, int8_t value){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_set_i8 fail: %s %s", key, nvs_error(err));
        return 0;
    }
    err = nvs_commit(_handle);
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_commit fail: %s %s", key, nvs_error(err));
        return 0;
    }
    return 1;
}
...
size_t Preferences::putUChar(const char* key, uint8_t value){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_set_u8 fail: %s %s", key, nvs_error(err));
        return 0;
    }
    err = nvs_commit(_handle);
    if(err){
...
size_t Preferences::putShort(const char* key, int16_t value){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_set_i16 fail: %s %s", key, nvs_error(err));
        return 0;
    }
    err = nvs_commit(_handle);
    if(err){
...
size_t Preferences::putUShort(const char* key, uint16_t value){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_set_u16 fail: %s %s", key, nvs_error(err));
        return 0;
    }
    err = nvs_commit(_handle);
    if(err){
...
size_t Preferences::putInt(const char* key, int32_t value){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_set_i32 fail: %s %s", key, nvs_error(err));
        return 0;
    }
    err = nvs_commit(_handle);
    if(err){
...
size_t Preferences::putUInt(const char* key, uint32_t value){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_set_u32 fail: %s %s", key, nvs_error(err));
        return 0;
    }
    err = nvs_commit(_handle);
    if(err){
...
size_t Preferences::putLong64(const char* key, int64_t value){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_set_i64 fail: %s %s", key, nvs_error(err));
        return 0;
    }
    err = nvs_commit(_handle);
    if(err){
...
size_t Preferences::putULong64(const char* key, uint64_t value){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_set_u64 fail: %s %s", key, nvs_error(err));
        return 0;
    }
    err = nvs_commit(_handle);
    if(err){
...
size_t Preferences::putString(const char* key, const char* value){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_set_str fail: %s %s", key, nvs_error(err));
        return 0;
    }
    err = nvs_commit(_handle);
    if(err){
...
size_t Preferences::putBytes(const char* key, const void* value, size_t len){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_set_blob fail: %s %s", key, nvs_error(err));
        return 0;
    }
    err = nvs_commit(_handle);
    if(err){
...
int8_t Preferences::getChar(const char* key, const int8_t defaultValue){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_get_i8 fail: %s %s", key, nvs_error(err));
    }
    return value;
}
...
uint8_t Preferences::getUChar(const char* key, const uint8_t defaultValue){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_get_u8 fail: %s %s", key, nvs_error(err));
    }
    return value;
}
...
int16_t Preferences::getShort(const char* key, const int16_t defaultValue){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_get_i16 fail: %s %s", key, nvs_error(err));
    }
    return value;
}
...
uint16_t Preferences::getUShort(const char* key, const uint16_t defaultValue){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_get_u16 fail: %s %s", key, nvs_error(err));
    }
    return value;
}
...
int32_t Preferences::getInt(const char* key, const int32_t defaultValue){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_get_i32 fail: %s %s", key, nvs_error(err));
    }
    return value;
}
...
uint32_t Preferences::getUInt(const char* key, const uint32_t defaultValue){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_get_u32 fail: %s %s", key, nvs_error(err));
    }
    return value;
}
...
int64_t Preferences::getLong64(const char* key, const int64_t defaultValue){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_get_i64 fail: %s %s", key, nvs_error(err));
    }
    return value;
}
...
uint64_t Preferences::getULong64(const char* key, const uint64_t defaultValue){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_get_u64 fail: %s %s", key, nvs_error(err));
    }
    return value;
}
...
size_t Preferences::getString(const char* key, char* value, const size_t maxLen){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_get_str len fail: %s %s", key, nvs_error(err));
        return 0;
    }
    if(len > maxLen){
        esp_log_write(ESP_LOG_ERROR, TAG, "not enough space in value: %u < %u", maxLen, len);
        return 0;
    }
    err = nvs_get_str(_handle, key, value, &len);
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_get_str fail: %s %s", key, nvs_error(err));
        return 0;
    }
    return len;
}
...
size_t Preferences::getBytes(const char* key, void * buf, size_t maxLen){
    ...
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_get_blob len fail: %s %s", key, nvs_error(err));
        return 0;
    }
    if(len > maxLen){
        esp_log_write(ESP_LOG_ERROR, TAG, "not enough space in buffer: %u < %u", maxLen, len);
        return 0;
    }
    err = nvs_get_blob(_handle, key, buf, &len);
    if(err){
        esp_log_write(ESP_LOG_ERROR, TAG, "nvs_get_blob fail: %s %s", key, nvs_error(err));
        return 0;
    }
    return len;
}
